...
links.forEach((item, index)=>{
  item.addEventListener('click', (e)=>{
    ...
    document.querySelector('.portfolio_pop').style.display = 'block'

    // 클릭한 li의 index를 가져온다
    console.log('index', index)

    // portfolioData[index].subject 데이터를 팝업의 h1태그에 바인딩 한다
    document.querySelector('.portfolio_pop h1').innerHTML = portfolioData[index].subject

    // portfolioData[index].desc 데이터를 팝업의 p태그에 바인딩 한다
    document.querySelector('.portfolio_pop p').innerHTML = portfolioData[index].desc
    
    // portfolioData[index].imgSrc 데이터를 팝업의 img 태그에 속성으로 넣어준다
    document.querySelector('.portfolio_pop img').setAttribute('src', portfolioData[index].imgSrc[0])
